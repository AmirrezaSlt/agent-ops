/**
 * Service for handling chat API interactions
 */
...
export const sendChatRequest = async (message, messageHistory = [], abortController = null, conversationId = null) => {
  ...
  if (!response.ok) {
    ...
    if (response.status === 404) {
      errorMessage = `API endpoint not found: ${chatEndpoint}. This could mean the backend API is not configured correctly.`;
...
/**
 * Processes a streaming response from the chat API
